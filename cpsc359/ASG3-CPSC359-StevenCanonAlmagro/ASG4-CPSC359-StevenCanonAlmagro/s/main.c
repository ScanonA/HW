...
void refreshSquare(int row, int col, int squareSize) {
  ...
  if(maze[row][col] == 0) {
    ...
    drawSquare(row*squareSize, col*squareSize, squareSize, RED2);
  }
}
...
void changeMazeValue(int y, int x, int num) {
  maze[y][x] = num;
}
  ...
  if ((data != currentState)) {
      ...
      if ( (data & BUTTON_LEFT) == BUTTON_LEFT ) {
        if ((playerRow > 0) && checkCollision(playerRow-1, playerCol) == 0) {
          if(wallTypeCheck(playerCol, playerRow-1) == 0) {
            changeMazeValue(playerCol, playerRow-1, 10);
            changeMazeValue(playerCol, playerRow, 10);
            refreshSquare(playerCol, playerRow-1, 64);
          }
          else if(wallTypeCheck(playerCol, playerRow-1) == 10) {
            changeMazeValue(playerCol, playerRow, 0);
            refreshSquare(playerCol, playerRow-1, 64);
          }
          refreshSquare(playerCol, playerRow, 64);
          playerRow -= 1;
      ...
      if ( (data & BUTTON_UP) == BUTTON_UP ) {
        if((playerCol > 0) && checkCollision(playerRow, playerCol-1) == 0 ) {
          if(wallTypeCheck(playerCol-1, playerRow) == 0) {
            changeMazeValue(playerCol-1, playerRow, 10);
            changeMazeValue(playerCol, playerRow, 10);
            refreshSquare(playerCol-1, playerRow, 64);
          }
          else if(wallTypeCheck(playerCol-1, playerRow) == 10) {
            changeMazeValue(playerCol, playerRow, 0);
            refreshSquare(playerCol-1, playerRow, 64);
          }
          refreshSquare(playerCol, playerRow, 64);
